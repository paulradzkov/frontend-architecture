...
> #### Делать конструктор из мелких деталей
...
1. **Чтобы переменные были локальными**
    ...
    Локальные переменные не смешиваются с переменными других компонентов и можно в каждом компоненте смело использовать простые имена — `@color`, `@background`, `@border`, `@width` — и не париться насчет конфликтов.

2. **Компонент можно перенастроить через миксин**

    В LESS есть такая особенность, если объявить в разных местах два миксина с одинаковым именем, то они сольются в один миксин, причем переменные и css-свойства из последнего переопределят таковые из первого миксина. Чистые css-свойства продублируются, но приоритет будет у последних согласно порядку следования в коде.
...
## Как импортировать и перенастроить компонент в проекте

Пусть на проекте такая файловая структура:
...
Внутрь папки `components` мы скопировали less-файлы нужных нам компонентов.

В главном файле проекта `project.less` в нужном порядке подключаем файлы компонентов и последним подключаем файл с настройками проекта `_settings.less`.

```less
// project.less

...
@import (less) "components/_mycomponent.less";
@import (less) "components/_mycomponent-2.less";
...

@import (less) "_settings.less"; // should be last

// project.less EOF
```

В файле настроек могут быть глобальные переменные проекта и переопределения настроек компонентов. Порядок следования миксинов не важен, главное чтобы миксин с настройками вызывался после импорта своего файла компонента. Все переопределения «всплывут» в конце своих файлов компонентов.
...
// some global var
@brand-color: red;
@brand-alt-color: orange;
...

// redefine parameters
.mycomponent-settings() {
    @color: @brand-color; // local variable mapped to global variable
    font-weight: normal;  // redefined non-variable property

    // we can do hotfix for elements
    &-part {
...
Мы можем сопоставить существующим внутри компонентов локальным переменным глобальные переменные нашего проекта, добавить новые css-свойства или переопределить существующие как для корневого класса так и для других элементов компонента.

> #### Глобальные переменные используется в только в пределах файла, где они объявлены

Стало легче поддерживать порядок в глобальных переменных, когда они не используются за пределами своего файла.

## Как переименовать компонент с помощью глобальной переменной

Не все проекты пишутся с нуля, и часто в проекте уже есть какой-то CSS код. Может случиться ситуация, когда имя компонента уже занято каким-то классом.

Переименуем базовый класс компонента и избавимся от конфликта имён.

Для этого в каждом компоненте заготовлена глобальная переменная с названием компонента: `@mycomponent: mycomponent;`.

В проектном файле после импорта файла компонента переопределить глобальную переменную, чтобы получилось так:
...
В скомпилированном CSS будет `.newname { ... }` вместо `.mycomponent { ... }`.
